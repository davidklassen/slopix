...
TEST(fs_iget_root) {
	struct inode *ip = fs_iget(0, ROOTINO);
...
TEST(fs_iget_same_twice) {
	struct inode *ip1 = fs_iget(0, ROOTINO);
	int ref1 = ip1->ref;
	struct inode *ip2 = fs_iget(0, ROOTINO);
...
TEST(fs_bmap_root_direct) {
	struct inode *ip = fs_iget(0, ROOTINO);
	fs_ilock(ip);
	unsigned int bn = fs_bmap(ip, 0);
	ASSERT(bn > 0, "fs_bmap(0) should return valid block number");
	fs_iunlock(ip);
	fs_iput(ip);
	return 0;
}
...
TEST(fs_dir_dot) {
	...
	fs_ilock(root);
	struct inode *dot = fs_dirlookup(root, ".", 0);
...
TEST(fs_dir_dotdot) {
	struct inode *root = fs_iget(0, ROOTINO);
	fs_ilock(root);
	struct inode *dotdot = fs_dirlookup(root, "..", 0);
...
TEST(fs_namei_root) {
	...
	ASSERT_NOT_NULL(ip, "fs_namei('/') should return non-null");
	ASSERT_EQ(ip->inum, ROOTINO, "fs_namei('/') should return root inode");
	fs_ilock(ip);
...
TEST(fs_namei_file) {
	...
	ASSERT_NOT_NULL(ip, "fs_namei('/hello') should return non-null");
	fs_ilock(ip);
...
TEST(fs_namei_relative) {
	...
	fake_proc.cwd = fs_iget(0, ROOTINO);
...
TEST(fs_namei_relative_dot) {
	...
	struct inode *ip = fs_namei(".");
	ASSERT_NOT_NULL(ip, "fs_namei('.') should return non-null");
	ASSERT_EQ(ip->inum, ROOTINO, "fs_namei('.') should return root");
...
TEST(fs_readi_small) {
	...
	fs_ilock(ip);
	char buf[64];
	int n = fs_readi(ip, buf, 0, sizeof(buf));
