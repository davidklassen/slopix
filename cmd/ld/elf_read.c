...
static void elf_check_header(Elf64_Ehdr *ehdr, const char *filename) {
...
static void elf_parse_sections(ObjectFile *obj) {
	obj->shnum = obj->ehdr->e_shnum;
	if (obj->ehdr->e_shoff + obj->shnum * sizeof(Elf64_Shdr) > obj->size) {
		error("%s: section headers extend past end of file", obj->filename);
	}
...
ObjectFile *elf_read(const char *path) {
	...
	obj->is_archive_member = false;

	elf_check_header(obj->ehdr, path);
	elf_parse_sections(obj);
...
ObjectFile *elf_read_memory(uint8_t *data, size_t size, const char *name) {
	if (size < sizeof(Elf64_Ehdr)) {
		error("%s: file too small for ELF header", name);
	}
	...
	obj->size = size;
	obj->ehdr = (Elf64_Ehdr *)data;
	obj->is_archive_member = true;

	elf_check_header(obj->ehdr, name);
	elf_parse_sections(obj);
