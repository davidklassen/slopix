...
static unsigned long *l1_table;        // TTBR0 Level 1 table (starting level, pointed to by TTBR0)
static unsigned long *l2_table_low;    // TTBR0 Level 2 for first 1GB (0x00000000-0x3FFFFFFF)
static unsigned long *l2_table_kernel; // TTBR0 Level 2 for second 1GB (0x40000000-0x7FFFFFFF)

// TTBR1 page tables for higher-half kernel mapping (0xFFFF_FF80_4000_0000+)
static unsigned long *ttbr1_l1_table;        // TTBR1 Level 1 table (starting level, pointed to by TTBR1)
static unsigned long *ttbr1_l2_table_low;    // TTBR1 Level 2 for first 1GB
...
void mmu_init(void) {
    ...
    if (!l1_table || !l2_table_low || !l2_table_kernel) {
        printf("[MMU] FATAL: Failed to allocate TTBR0 page tables\n");
    ...
    if (!ttbr1_l1_table || !ttbr1_l2_table_low || !ttbr1_l2_table_kernel) {
        printf("[MMU] FATAL: Failed to allocate TTBR1 page tables\n");
        while (1) __asm__ volatile("wfe");
    }

    // Zero all TTBR0 page table entries (PAGE_TABLE_ENTRIES entries * 8 bytes = 4096 bytes each)
    ...
    // Fill L2_low with identity mapping for first 1GB (PAGE_TABLE_ENTRIES entries * 2MB each)
    for (unsigned int i = 0; i < PAGE_TABLE_ENTRIES; i++) {
        unsigned long phys_addr = ((unsigned long)i) << L2_BLOCK_SHIFT;  // i * 2MB

        // Use device memory for MMIO regions, normal memory for RAM
        unsigned long attr;
        if (phys_addr >= DEVICE_REGION_START && phys_addr < DEVICE_REGION_END) {
